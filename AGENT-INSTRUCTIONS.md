...
3. **Step 3: Scaffold from Templates**
   Do not hallucinate the documentation structure. You MUST read the exact structures from the `templates/` directory.
   - For every file in the `templates/` directory, there is a `<template target_path="[path]">` tag.
   - You MUST extract the markdown payload from inside that tag and save it to the target project using the exact `target_path` specified (e.g., `docs/architecture/01_system_design.md`).
